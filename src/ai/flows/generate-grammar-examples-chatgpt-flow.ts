'use server';
/**
 * @fileOverview A Genkit flow for generating additional English example sentences using ChatGPT.
 *
 * - generateGrammarExamplesWithChatGPT - A function that handles the generation of grammar examples via ChatGPT.
 * - GenerateGrammarExamplesInput - The input type for the generateGrammarExamplesWithChatGPT function.
 * - GenerateGrammarExamplesOutput - The return type for the generateGrammarExamplesWithChatGPT function.
 */
...
export async function generateGrammarExamplesWithChatGPT(input: GenerateGrammarExamplesInput): Promise<GenerateGrammarExamplesOutput> {
  return generateGrammarExamplesChatGPTFlow(input);
}

const generateGrammarExamplesChatGPTPrompt = ai.definePrompt({
  name: 'generateGrammarExamplesChatGPTPrompt',
...
const generateGrammarExamplesChatGPTFlow = ai.defineFlow(
  {
    name: 'generateGrammarExamplesChatGPTFlow',
    inputSchema: GenerateGrammarExamplesInputSchema,
    outputSchema: GenerateGrammarExamplesOutputSchema,
  },
  async (input) => {
    const { output } = await generateGrammarExamplesChatGPTPrompt(input);
    return output!;
  }
);
